...
namespace Corby.Framework.UI
{
    ...
    [Serializable]
    public abstract class Presenter<TView> : IPresenter where TView : Component
    {
        [ShowInInspector]
        ...
        public TView View { get => _view; protected set => _view = value; }
        ...
        {
            _view = Object.FindFirstObjectByType<TView>(FindObjectsInactive.Include);
            if (_view == null)
            {
                Debug.LogWarning(@$"'{GetType().Name}' 프레젠터가 View({typeof(TView).Name})을 찾을 수 없습니다.
